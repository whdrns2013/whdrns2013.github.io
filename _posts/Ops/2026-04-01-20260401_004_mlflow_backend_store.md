...
lastmod: 2026-06-14 17:43:00 +0900   # 최종 수정일 (필수)
last_modified_at: 2026-06-14 17:43:00 +0900   # 최종 수정일 (필수)
...
Docker Compose 를 이용해 PostgreSQL Backend Store를 구동해보자.  

```yaml
# docker-compose.mlflow-postgres.yml
volumes:
  mlflow-pgdata:
...
Docker Compose 명령을 통해 컨테이너와 서비스를 실행한 뒤, DB에 접속해보자.  

```bash
sudo docker compose -f docker-compose.mlflow-postgres.yml up -d
```

아직 별다른 테이블은 생성되지 않았다. 추후 MLflow Tracking Server 와 연결하면 관련 테이블들이 생길 것이다.  

![](/assets/images/20260401_004_001.png)
