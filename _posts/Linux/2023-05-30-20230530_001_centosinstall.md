...
이용을 원하는 버전 -> isos -> DVD 버전을 다운로드 하면 된다.  
...
아주 같은 예는 아니지만, Virtual Box는 이와 비슷하게 현재 컴퓨터 위에 "가상 OS"를 설치하는 것으로 이해하면 편하다.  
...
가상머신은 Windows 위에서 돌아가며, 가상머신과 Windows를 왔다갔다 하려면 이 "탈출 키" 지정이 필수적이다.  
...
root 계정으로 로그인하려면 "목록에 없습니까?" 를 선택하고, 계정 란에 root를 입력한 후 진행하면 된다.  
