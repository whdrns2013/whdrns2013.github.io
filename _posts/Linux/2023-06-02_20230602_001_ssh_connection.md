...
이번 포스트에서 호스트 머신의 OS는 윈도우, 게스트(원격) 머신의 OS는 리눅스(Cent, Ubuntu) 환경입니다.  
...
기존의 원격 접속 방법이 암호화가 되지 않아 보안성이 약했던 반면, SSH는 암호화 기법을 사용합니다.  
...
이제 SSH 연결을 해봅니다.  
SSH 연결은 게스트 머신이 아닌, 호스트 머신에서 합니다.  

호스트에서 콘솔창을 띄워주신 후, ssh 접속 명령어를 입력합니다.  
...
나는 기본 콘솔을 선호한다. 뭔가 있어보이기도 하고
실력적으로도 기본 콘솔에 익숙해야
어떠한 상황에서든 작업이 가능하기 때문

하지만 유용한 툴들이 많기 때문에 소개
