...
|대상|옵션|설명|예시|
|---|---|---|---|
|column|NOT NULL|칼럼에 null 값을 허용하지 않음. 생략 가능하며, 생략시 null 값을 허용한다.|CREATE TABLE test_table (column1 varchar(20) not null);<br>ALTER TABLE test_table (column2 varchar(20) not null);|
|column|RENAME COLUMN TO|칼럼명을 변경한다.|ALTER TABLE test_table RENAME COLUMN (column_origib) TO (column_new);|
|column|MODIFY|칼럼의 자료형, null 여부 등 설정사항을 변경한다.|ALTER TABLE test_table MODIFY (column1) tinyint NOT NULL;|
