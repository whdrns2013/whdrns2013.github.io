...
개인적으로 모델을 서빙할 때 자주 사용하는 방식이기도 하며, 특히나 **MLflow와 같은 외부 모델 레지스트리를 이용할 경우**, 또는 **다중 worker**를 띄우면서 모델 다운로드는 한 번만 하고, 각 워커에서는 다운로드한 모델을 로딩하여 **초기 서비스 실행 속도를 높일 때 사용**한다.  
