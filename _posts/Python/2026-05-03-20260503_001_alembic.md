---
title: "[Python] Alembic으로 구축하는 현대적 Python DB 워크플로우" # 제목 (필수)
...
> - 그런데 이렇게 테이블 이름이 변경된 경우, 주의해야 할 점이 있다.  
> - 변경사항을 적용하면 테이블 이름이 바뀌는 게 아니라, 새로운 테이블을 만들고 예전 이름 테이블을 DROP 해버린다는 점.  
> - 따라서 데이터가 소실될 수 있으니, 테이블명 변경 마이그레이션은 반드시 주의해서 적용해야 한다.
