...
lastmod: 2026-07-08 00:56:00 +0900   # 최종 수정일 (필수)
last_modified_at: 2026-07-08 00:56:00 +0900   # 최종 수정일 (필수)
...
class User(Base):
    ...
    # 그 외 테이블 설정
    __table_args__ = {
        "schema": "service_a" # e.g. postgres : 이 테이블이 속한 스키마 지정
...
| `default` | default=datetime.utcnow() | Python 쪽 기본값 (SQLAlchemy 단에서 Insert할 때 적용)|
| `server_default` | server_default=text("now()") | DB 서버 기본값 (DB에 Insert될 때 적용)|
| `onupdate` | onupdate=datetime.utcnow | UPDATE 시 자동 값 |
| `server_onupdate` |server_onupdate=text("now()") | UPDATE 시 DB 서버에서 자동으로 교체할 값|
...
SQLAlchemy 에서 각 컬럼의 기본값을 지정하는 방식은 세 가지가 있다.  

| 옵션               | 핵심 의미                | 실행 주체 | 대표 사용 예           |
| ---------------- | -------------------- | ---------|-------- |
| `default`        | INSERT 때 기본값을 넣음    | 클라이언트 | 생성 시간, 기본 상태값     |
| `server_default` | INSERT 때 기본값을 넣음 | DB 서버 | DB 기준 생성 시간       |
| `onupdate`       | UPDATE 때 자동으로 값을 바꿈  | 클라이언트 | 수정 시간, 수정 상태, 버전값 |
| `server_onupdate` | UPDATE 때 자동으로 값을 바꿈 | DB 서버 | 수정 시간, 수정 상태, 버전값       |

- `default` 는 파이썬 코드에서 기본값을 생성해서 DB로 넘기게 되며  
- `server_default` 는 DB에 Insert가 일어날 때 DB가 직접 기본값을 생성해서 넣는다.  

#### g. 시간 기본값 설정 방법  

- 파이썬이 INSERT 전에 시간을 넣는 방식  
...
updated_at = Column(DateTime, server_default=func.now(), server_onupdate=text("CURRENT_TIMESTAMP"))
```
...
class CommonCodeGroup(Base):
    ...
    group_code : Mapped[str] = mapped_column(String(100), primary_key=True, comment="그룹코드")
    comment    : Mapped[str] = mapped_column(String(400), comment="그룹코드설명")

class CommonCode(Base):
    '''공통코드'''
    __tablename__ = 'common_code'
    group_code : Mapped[str] = mapped_column(ForeignKey("manage_common_code_group.group_code"),
                                             primary_key=True, comment="그룹코드")
    code       : Mapped[str] = mapped_column(String(100), primary_key=True, comment="코드")
    comment    : Mapped[str] = mapped_column(String(400), comment="코드설명")
```
...
Users.__table__.create(bind=engine) # <-- 이렇게
```

그러면 users 테이블만 생성된 것을 볼 수 있다.

![](/assets/images/20260427_001_005.png)
...
- 해당 테이블의 데이터를 **전체 삭제**하려면 아래를 참고한다.  
...
- SQLAlchemy 와 함께 직접 텍스트로 된 쿼리문을 사용할 수도 있다.
...
with target_engine.begin() as conn:
    # f-string으로 만든 문자열을 text()로 감싸줌
    conn.execute(text(f"RENAME TABLE {tb1} TO {'drop_' + tb1};"))
    conn.execute(text(f"RENAME TABLE {tb2} TO {tb1};"))
    conn.execute(text(f"DROP TABLE {'drop_' + tb1};"))
```

- 데이터에 대한 CRUD의 경우, 하나의 컨텍스트 안에서 수행되는 작업은 **단일 트랜잭션으로 취급**된다.
- 단, 위처럼 DDL문인 경우에는 DBMS에 따라 트랜잭션 처리가 다르므로 주의!
